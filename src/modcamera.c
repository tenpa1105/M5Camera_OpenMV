...
STATIC bool camera_init_helper(camera_obj_t *camera, size_t n_pos_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    ...
    static const mp_arg_t allowed_args[] = {
        ...
        { MP_QSTR_format,          MP_ARG_KW_ONLY                   | MP_ARG_INT,   {.u_int = PIXFORMAT_GRAYSCALE} },
...
STATIC mp_obj_t camera_capture(){
    ...
    uint8_t *buf;
    image_t *img_cobj = (image_t *)py_image_cobj(image);
    switch (fb->format){
    ...
    buf = (uint8_t*)m_new(uint8_t, fb->len);
    memcpy(buf, fb->buf, fb->len);
    img_cobj->data = buf;
    img_cobj->w = fb->width;
    img_cobj->h = fb->height;
    esp_camera_fb_return(fb);
    return image;
}
